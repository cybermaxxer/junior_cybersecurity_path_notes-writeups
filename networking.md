Pre scriptum: this md also pulls in info from outside sources that complements HTB's own material.
...
the whole module boils down to this: internet protocols are just agreed-on rules (RFCs) so any two devices can talk regardless of vendor. everything downstream, ports, addressing, VLANs, exists to answer one question: how does data get from A to B reliably and identifiably.
...
| Kerberos | 88 | AD authentication (named after the 3-headed dog, not an acronym) |
...
**SIP/VoIP:** runs on TCP/UDP 5060-5061. the `OPTIONS` method is the offensively interesting one, sending it to different usernames enumerates valid users on a SIP server (different response for valid vs invalid), which sets up brute forcing.
...
- IP delivery on the local network actually happens via MAC, not IP, layer 2 needs the physical address to hand off a frame
...
**ARP (Address Resolution Protocol):** resolves layer 3 IP → layer 2 MAC. a device broadcasts "who has X IP", the owner replies with its MAC. this is how devices go from "I know your IP" to "I can actually send you an ethernet frame."

**ARP spoofing/cache poisoning** (pentest relevant): send falsified ARP replies claiming your MAC owns someone else's IP, usually the gateway. the victim's ARP cache gets poisoned, traffic meant for the gateway now routes through the attacker, MITM. tools: Ettercap, Cain & Abel.

Q: why does ARP spoofing work at all, what's the actual flaw being exploited? A: ARP has no authentication, any device can claim to own any IP, so a forged reply gets accepted at face value.
...
| MAC filtering bypass | if the network only allows specific MACs, spoof an allowed one |
...
**why hex specifically:** decimal shows 10 states per char, binary shows 2, hex shows 16, it's the most compact human-readable way to represent long binary strings. IPv6 is 128 bits = 8 blocks of 16 bits (4 hex digits each), separated by `:` instead of `.`.

**shortening rules (RFC 5952)**, this part is arbitrary convention, just memorize it:
...
**structure:** network prefix (network part) + interface identifier/suffix (host part). default prefix is `/64`. the interface identifier is derived from the 48-bit MAC, expanded to 64 bits.

Q: why can IPv6 skip NAT entirely? A: the address space is so large every device can get a real public address, no need to conserve via private+NAT translation.
...
2. `CIDR mod 8` = remainder, that tells you the block size via `256 / 2^remainder`
...
**dividing into N subnets:** subnet count must be a power of 2. extend the mask by however many bits satisfy `2^n = required subnets`. need 4 subnets → extend the mask by 2 bits (`2^2=4`).
...
the full journey when you type a URL, step by step (this ties every other topic together):

1. **connect to WLAN**, SSID selected, WPA2/3 password auth, DHCP takes over IP config
2. **DHCP**, laptop gets a private IP (e.g. `192.168.1.10`) plus subnet mask, gateway, DNS server
3. **DNS resolution**, query goes to a DNS server (ISP/Google DNS), response returns the real IP of the site
4. **encapsulation** (OSI/TCP-IP model, top-down):
   - application layer: browser builds the HTTP/HTTPS request
   - transport layer: wrapped in a TCP segment (ports 80/443)
   - internet layer: wrapped in an IP packet (source = private IP, dest = remote server IP)
   - link layer: wrapped in an Ethernet/WiFi frame (source/dest MAC), gateway's MAC found via ARP if not already cached
5. **NAT at the router**: private IP swapped for the router's public IP in the packet header, forwarded to the ISP
6. **server response**: firewall checks the port, the web server (Apache/Nginx/IIS) processes and replies, reverse path back through the internet
7. **NAT unwinds** at the home router (maps the public IP back to the private IP)
8. **decapsulation**: laptop strips the frame, then the IP header, then the TCP header, browser renders the HTML/CSS/JS

Q: why does the laptop need ARP before it can even send the first packet out to the internet? A: it needs the gateway's MAC address to build the link-layer frame. knowing the destination IP isn't enough, you still need the local physical hop's MAC to transmit anything at all.

---

### 1.4 IP packet anatomy + sniffing/traceroute

**IP packet, envelope model.** header = sender/recipient/routing info, payload = the actual data (the letter inside).
...
**IP ID as a fingerprinting trick:** if one host has multiple IPs, its outbound packets from different source IPs will still show a very close/sequential IP ID counter, since it's the same OS TCP/IP stack generating them. continuous IDs across two "different" IPs is a strong signal they're the same physical host (pentest relevant: host discovery/correlation).
...
1. send a TCP SYN with TTL=1
2. the first router decrements TTL to 0, drops the packet, replies with ICMP Time-Exceeded
3. note that router's IP, resend with TTL+1
4. repeat until you get a SYN/ACK or RST from the actual destination
...
**UDP traceroute** (common on unix hosts): gets Destination/Port Unreachable instead of SYN/ACK once it finally reaches the target.

**Blind spoofing:** attacker forges the source IP/port and a fake Initial Sequence Number (ISN) in a TCP packet without seeing the real responses, trying to trick the target into establishing a connection state with a spoofed identity. it's "blind" because the attacker never sees the actual reply traffic.
...
| shared key | AP sends a plaintext challenge, client encrypts it with the WEP key + RC4, AP verifies, this leaks free keystream to any sniffer |
...
6. IV sent in cleartext alongside the ciphertext

**why it collapses:** the 24-bit IV space is only 16.7M values, cycles in hours on a busy AP. IV reuse → identical keystream → `ciphertext_A XOR ciphertext_B = plaintext_A XOR plaintext_B`. combined with predictable wifi traffic patterns (ARP, DHCP), tools like aircrack-ng statistically recover the static key itself (FMS attack, KoreK/PTW improvements).

mental model: reusing a static key with only a tiny IV to disguise it is structurally guaranteed to fail given enough traffic, this isn't "a clever trick was found once."

**mnemonic**: SIRKX → Seed, ICV, RC4 keystream, Key reuse, Xor collision.

Q: why does IV reuse leak info even without knowing the WEP key? A: identical seed → identical keystream → XOR of two ciphertexts cancels the keystream, leaving the XOR of the two plaintexts, no key knowledge needed for that step.
...
phase 1 (joining/4-way handshake) is identical to WPA2, see below. the whole difference is in phase 2: how the per-packet key gets generated.
...
| Michael MIC | real cryptographic integrity check, replaces the fake CRC32 | actually detects tampering |

**mental model:** WEP's flaw was architectural, a bad lock. WPA doesn't replace the lock, it rotates the pins slightly every use, better, but still trusting a fundamentally old design underneath. still built on RC4, and the Michael MIC was deliberately weakened for speed, so it's still crackable via chopchop-derived attacks, just not total key recovery like WEP.
...
| | ptk derived | alice | `ptk = prf(pmk, anonce, snonce, mac_ap, mac_alice)` | alice goes first, she now has all 5 ingredients |
| | message 2 | alice → ap | sends snonce + mic (built from kck) | proves she derived the correct ptk without sending it |
| | ptk derived | ap | same prf, now has both nonces | verifies alice's mic matches its own |
| | message 3 | ap → alice | sends gtk (wrapped with kek) + its own mic | ap proves itself back, mutual proof, delivers the broadcast key |
...
| **5. group traffic** | gtk usage | both | broadcast/multicast frames encrypted with the shared gtk instead of tk | everyone needs to decrypt the same broadcast frame |
...
phase 1 is byte-for-byte identical to WPA's 4-way handshake (above). phase 2 is where the actual innovation happens:
...
| AAD built | parts of the MAC header that need integrity but not secrecy | tamper protection without hiding routing info |
...
**why this actually closes the door:** AES is a block cipher with no practical break, and integrity is baked into the same cipher operation (AEAD = authenticated encryption with associated data) instead of bolted on separately like WEP/WPA. the remaining attack surface shifts entirely to weak passphrases, not the crypto.
...
| MIC | message integrity code | proves message authenticity, computed with KCK |
...
Q: why mix the MAC address into PTK derivation instead of just PMK+nonces? A: it ties the key to this specific device pair, even a nonce coincidence between two different devices would still produce different PTKs.

**WPA2 attack tooling** (the crypto itself is never attacked, the target is the password):

| tool | job |
|---|---|
| airodump-ng | monitors for a client (re)connecting, captures the 4-way handshake |
| aireplay-ng (deauth) | forges deauth frames, 802.11 mgmt frames are unauthenticated, forces a reconnect on demand |
| hashcat / aircrack-ng | offline crack: tries password candidates through PBKDF2→PMK→PTK→MIC, checks against the captured MIC |
| PMKID attack (hcxtools) | grabs the PMKID directly from the AP, skips needing a full handshake or connected client at all |
...
flow: 802.1X blocks the port until EAP auth completes → EAP method negotiated (PEAP/MSCHAPv2 for passwords, or EAP-TLS for certs) → RADIUS sends "access accept" + MSK (becomes PMK) → same 4-way handshake as personal from here on. phase 2 encryption is identical to personal too, enterprise only changes who gets to prove they belong.

**mental model:** the AP is a bouncer who doesn't know anyone, it just radios a name to the manager (RADIUS) inside who checks a list and radios back yes or no.
...
**core problem it fixes:** WPA2's PMK is a pure local computation (`PBKDF2(PSK,SSID)`), an attacker captures one handshake, then brute forces offline forever at unlimited speed. WPA3 replaces this with **SAE** (Simultaneous Authentication of Equals, aka the Dragonfly handshake), every single password guess requires a live round trip with the AP, so offline cracking is dead.
...
WEP-40/64 uses a 24-bit IV + 40-bit secret key, WEP-104 uses a 24-bit IV + 80-bit secret key. the CRC-based ICV has a structural flaw: it's calculated over plaintext, meaning you can determine plaintext content even from encrypted data via the CRC relationship, this is on top of the IV-reuse collapse above.

**LEAP vs PEAP:** LEAP uses a shared key for both encryption and auth (weak, RC4), PEAP tunnels through TLS first (strong). PEAP beats LEAP because PEAP encrypts the MSCHAPv2 hash exchange, LEAP doesn't.
...
**mental model:** a VLAN turns one physical switch into multiple logical switches, each VLAN is its own broadcast domain, so broadcast traffic in one VLAN never crosses into another without a router. that's why it's a security boundary, not just an organizational one.

- VLAN IDs: 1-4094 usable (0 and 4095 reserved). 1 is the default VLAN. 1-1005 = normal range (saved in `vlan.dat`), 1006-4094 = extended range (not saved).
...
**VLAN hopping attack:** exploits Cisco's DTP (Dynamic Trunking Protocol), which auto-negotiates trunk links between switches. an attacker configures a host to mimic a switch, spoofs DTP+802.1Q signaling on a port where DTP is enabled by default, and the switch establishes a trunk link with the attacker. now the attacker sees traffic from every VLAN, not just their assigned one. tool: Yersinia.

**double-tagging VLAN hopping** (only works if the attacker is on the trunk's native VLAN):

1. attacker sends a double-tagged frame: outer tag = native VLAN, inner tag = target VLAN
2. the first switch strips the outer tag (matches native VLAN, doesn't re-tag since native VLAN frames go untagged on a trunk), and never inspects the inner tag
3. the second switch now sees only the inner tag, forwards it to the target VLAN
...
**VLAN-capable NICs** (linux): load the `8021q` kernel module → `ip link add link eth0 name eth0.20 type vlan id 20` (or the deprecated `vconfig`) → assign an IP → bring the interface up.

**wireshark VLAN analysis:** filter `vlan` for tagged traffic, `vlan.id == 10` for a specific VLAN, `tshark -r file.pcapng -T fields -e vlan.id | sort -n -u` to enumerate all VLAN IDs seen in a capture.

---

### 1.7 VPNs, IPsec, key exchange, cryptography basics

**VPN core idea:** an encrypted tunnel between a remote device and a private network, so the remote machine can act as if it's locally connected. requires: VPN client, VPN server, encryption, mutual authentication.
...
| Tunnel | encrypts the entire packet including the header, network to network VPN tunnels |
...
|---|---|
| IKE | UDP/500 |
| ESP | IP protocol 50 (or UDP/4500 when NAT traversal is needed) |
| AH | IP protocol 51 |
...
- DES: 64-bit key, but 8 bits are a checksum, so effective 56-bit key. weak by modern standards.
...
| RSA | based on the difficulty of factoring large primes | widely used, heavier computationally than ECC at the same security level |
...
Q: why is plain Diffie-Hellman vulnerable to MITM even though the math itself is sound? A: DH alone has no built-in authentication, an attacker can intercept and run separate DH exchanges with each side, impersonating the other party to both, since nothing verifies who you're actually exchanging keys with.

**IKE (Internet Key Exchange):** combines DH with other crypto to negotiate VPN security parameters. two modes:
...
**CDP (Cisco Discovery Protocol):** a layer 2 protocol, Cisco devices broadcast info about themselves to directly connected Cisco neighbors (device ID, IP, port, OS version, hardware platform). useful for topology discovery, but also an information leak, often disabled for security.
...
1. **you capture a WPA2 handshake off the air but the password is genuinely strong (20+ random chars). can you crack it? why does WPA3 change this specific scenario?** → practically no, hashcat/aircrack still have to brute force PBKDF2→PMK→PTK→MIC offline, and a strong random password makes that computationally infeasible regardless of speed. WPA3 doesn't make strong passwords "more crackable," it makes any password (including weak ones) resistant, because SAE requires a live AP round trip per guess, so the offline math shortcut WPA2 allows simply doesn't exist in WPA3.
